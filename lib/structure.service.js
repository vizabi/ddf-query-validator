...
function validateWhereStructure(query, options) {
    ...
    errorMessages.push(checkIfWhereHasInvalidStructure(whereClause, getJoinIDPathIfExists(options)), checkIfWhereHasUnknownOperators(joinClause, whereOperators, getJoinIDPathIfExists(options)), checkIfBooleanOperatorsAreArrays(whereClause, getJoinIDPathIfExists(options)));
    return (0, compact_1.default)(errorMessages);
}
...
function validateJoinStructure(query, options) {
    ...
    switch (true) {
        ...
        case ((0, helper_service_1.isDatapointsQuery)(query)):
        default:
            errorMessages.push(checkIfJoinHasInvalidStructure(joinClause), ...(0, map_1.default)(joinClause, (item, joinID) => checkIfJoinKeyHasInvalidStructure(item, getJoinIDPathIfExists({ joinID }))), ...(0, map_1.default)(joinClause, (item, joinID) => checkIfBooleanOperatorsAreArrays((0, get_1.default)(item, 'where'), getJoinIDPathIfExists({ joinID }))));
            break;
    }
    return (0, compact_1.default)(errorMessages);
}
...
function checkIfBooleanOperatorsAreArrays(clause, joinPath) {
    if ((0, isNil_1.default)(clause) || !(0, isObject_1.default)(clause))
        return;
    for (const field in clause) {
        if ((0, includes_1.default)(['$and', '$or', '$nor'], field) && !(0, isArray_1.default)(clause[field])) {
            return `'${joinPath}where' clause: operator '${field}' must be an array`;
        }
        if ((0, isObject_1.default)(clause[field])) {
